...
Mark rule as SELECT and specify visible columns. By default, rows the user can't access are silently filtered out (filter mode).
...
Mark rule as SELECT and specify visible columns, but raise an explicit error instead of silently filtering rows (require mode).

```sql
auth_rules.select_strict(VARIADIC columns TEXT[])
```

Use this when you want unauthorized access attempts to fail with an error instead of returning no results. This is useful for single-resource fetches (for example, viewing a specific document) where an empty result could be confusing.
...
Returns a JSONB marker representing the authenticated user's ID. The
compiler converts this to `auth.uid()` at query time.

```sql
auth_rules.user_id_marker()
```

Use this inside `eq()` in rule definitions. Don't use `user_id()` in
rules — that returns the current UUID at definition time, not a marker
the compiler can interpret.

### auth_rules.user_id()

Returns the authenticated user's UUID directly (`auth.uid()`). This is
a runtime helper, not for use in rule definitions.
