...
Auth Rules stores its configuration in two tables:

The `auth_rules.claims` table stores claim definitions:
...
For more explicit error handling in views, Auth Rules provides `require` functions:
...
The `auth_rules.generated_objects` table tracks what was created:
