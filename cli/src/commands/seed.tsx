...
import { applySeedFiles, findSeedFiles, setVerbose, closeSupabasePool } from '../lib/pg-delta.js';
...
export async function seedCommand(options: SeedOptions): Promise<void> {
  ...
  if (seedFiles.length === 0) {
    if (options.json) {
      console.log(JSON.stringify({ status: 'error', message: 'No seed files found' }));
    } else {
      console.error(`\n${C.error}Error:${C.reset} No seed files found`);
      console.error(`  Expected paths: ${seedPaths.join(', ')}`);
      console.error(`  Create seed files in ${C.fileName}supabase/seeds/${C.reset}\n`);
    }
    process.exitCode = 1;
    return;
  }
  ...
  if (options.dryRun) {
    if (options.json) {
      ...
      console.log('');
      console.log(`${C.bold}supa seed${C.reset} ${C.secondary}— dry run${C.reset}`);
      console.log('');
      console.log(`${C.secondary}Would apply ${seedFiles.length} seed file(s):${C.reset}`);
  ...
  if (!options.json) {
    console.log('');
    console.log(`${C.bold}supa seed${C.reset} ${C.secondary}— seeding database${C.reset}`);
    console.log('');
    ...
    console.log(`${C.secondary}Files:${C.reset}   ${C.value}${seedFiles.length}${C.reset}`);
    console.log('');
  }
  ...
  try {
    ...
    if (options.json) {
      ...
      if (result.success) {
        console.log(`${C.success}✓${C.reset} Seeded ${C.value}${result.filesApplied}${C.reset} files successfully`);
      } else {
        console.log(`${C.warning}⚠${C.reset} Seeded ${C.value}${result.filesApplied}${C.reset}/${result.totalFiles} files`);
        for (const error of result.errors) {
          console.log(`  ${C.error}✗${C.reset} ${C.fileName}${error.file}${C.reset}: ${error.error}`);
        }
      }
      console.log('');
    }
    ...
    if (options.json) {
      ...
      console.error(`${C.error}✗${C.reset} Seed failed: ${error instanceof Error ? error.message : String(error)}`);
      console.log('');
    }
    process.exitCode = 1;
  } finally {
...
export async function seedStatusCommand(options: { json?: boolean }): Promise<void> {
  const cwd = process.cwd();
  const supabaseDir = join(cwd, 'supabase');
  ...
  if (options.json) {
    ...
    console.log('');
    console.log(`${C.secondary}Enabled:${C.reset} ${seedConfig.enabled ? `${C.success}yes${C.reset}` : `${C.error}no${C.reset}`}`);
    console.log(`${C.secondary}Paths:${C.reset}   ${seedConfig.paths.join(', ')}`);
    console.log('');
    
    if (seedFiles.length === 0) {
      console.log(`${C.warning}No seed files found${C.reset}`);
    } else {
      console.log(`${C.secondary}Found ${seedFiles.length} seed file(s):${C.reset}`);
      for (const file of seedFiles) {
        console.log(`  ${C.fileName}${relative(supabaseDir, file)}${C.reset}`);
      }
    }
