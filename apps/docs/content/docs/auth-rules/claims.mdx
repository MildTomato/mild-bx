...
A claim maps users to entities they're related to — orgs, teams,
folders, courses. It's a SQL query that returns these relationships
from your existing tables.

For example, a claim might say "user Alice is in orgs `org-1` and
`org-2`" or "user Bob is a member of team `team-5`." The claim
doesn't grant access — it just describes the relationship.
[Rules](/docs/auth-rules/rules) use claims to decide what operations
are allowed.
