...
Claims answer the question "what does this user have access to?" They're the foundation of Auth Rules — you define relationships once, then reference them in your rules.

## Creating a Claim

A claim is a SQL query that returns user-to-value mappings. The query must return `user_id` as the first column:
...
Claims can include additional information like roles. This is useful for role-based access control.

Include the role in your claim:
...
Then use `check()` to filter by role (see [Role-Based Access](/docs/check-claim)):
...
If users in a parent team should access child teams, use a recursive query:
...
For subscriptions or enrollments with expiration dates:
...
Claims query your tables in real-time. For best performance:
