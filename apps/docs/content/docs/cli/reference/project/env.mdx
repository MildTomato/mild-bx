...
- [`propagate`](#propagate) - Propagate environment variables to all healthy preview branches
...
| Option | Type | Description |
|--------|------|-------------|
| `--scope <SCOPE>` | string | Scope for the variable: production, preview, development, or branch (omit for universal default) |
...
```bash
supa project env set API_KEY "sk_live_123"
```
...
```bash
supa project env set API_KEY "sk_live_123" --scope production
```

```bash
supa project env set API_KEY "sk_test_123" --scope preview
```

```bash
supa project env set API_KEY "sk_dev_123" --scope development
```

```bash
supa project env set DEBUG "true" --scope branch --branch feat/my-feature
```
...
```bash
supa project env unset DEBUG --scope production
```

```bash
supa project env unset DEBUG --scope preview
```

```bash
supa project env unset DEBUG --scope development
```

```bash
supa project env unset DEBUG --scope branch --branch feat/my-feature
```
...
```bash
supa project env list --environment preview --branch feat/my-feature
```
...
```bash
supa project env propagate [options]
```
...
```bash
supa project env propagate
```

```bash
supa project env propagate --dry-run
```

```bash
supa project env propagate --branch feat/my-feature
```

```bash
supa project env propagate --yes
```
