...
A rule answers the question: "what can users do with this data?"

It specifies what operations (SELECT, INSERT, UPDATE, DELETE) are allowed
on a table, and under what conditions. Rules reference
[claims](/docs/auth-rules/claims) to know what each user has access to —
a claim defines the relationship, a rule enforces it.
...
- `eq(column, value)` — column equals a value, a `user_id_marker()`,
