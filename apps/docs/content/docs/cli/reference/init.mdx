---
title: "supa init"
description: "Initialize a new supabase project"
generated: true
---

<DemoVideo id="supa-init" />

Set up Supabase in the current directory.

Creates the `supabase/` folder structure. You can start with local development
(no account needed), connect to an existing project, or create a new one.

Run this once at the root of your repository to get started.

## Development modes

Run `supa init` and select how you'd like to develop.

### Connecting to the platform

Select "Connect to existing project" or "Create a new project" to link to
Supabase Platform. Both options walk you through organization, project,
schema management, and workflow profile selection.

<Tabs items={['Connect to existing', 'Create new project']}>
<Tab value="Connect to existing">

<DemoVideo id="supa-init--connect" />

</Tab>
<Tab value="Create new project">

<DemoVideo id="supa-init" />

</Tab>
</Tabs>

### Local development

Select "Local development" to scaffold the project structure without an
account. Run `supa init` again later to connect to the platform.

If you previously initialized locally, running `supa init` again offers to
connect the existing project.

<DemoVideo id="supa-init--local" />

## Created files

After initialization, your project has this structure:

```
your-project/
├── supabase/
│   ├── config.json      # Project configuration
│   ├── schema/          # SQL schema files (synced with remote)
│   ├── migrations/      # Version-controlled migrations
│   └── types/
│       └── database.ts  # Generated TypeScript types
└── .env.local           # API credentials (if new project)
```

## Usage

```bash
supa init [options]
```

## Options

| Option | Type | Description |
|--------|------|-------------|
| `-y, --yes` | boolean | Skip prompts and use defaults |
| `--local` | boolean | Initialize locally without connecting to Supabase Platform |
| `--org <SLUG>` | string | Organization slug |
| `--project <REF>` | string | Link to existing project by ref |
| `--new` | boolean | Create a new project (requires --org, --name, --region) |
| `--link <REF>` | string | Link to an existing project by ref |
| `--name <NAME>` | string | Name for new project (requires --org and --region) |
| `--region <REGION>` | string | Region for new project (e.g., us-east-1) |
| `--workflow-profile <PROFILE>` | string | Workflow profile: remote, local, branching-remote, branching-local (default: branching-remote) |
| `--schema-management <MODE>` | string | Schema management: declarative, migrations (default: declarative) |
| `--config-source <SOURCE>` | string | Config source: code, remote (default: code) |
| `--json` | boolean | Output as JSON |
| `--dry-run` | boolean | Preview what would be created without making changes |

### `--local`

Skip the interactive prompt and initialize locally without connecting to Supabase Platform. Creates the `supabase/` directory structure with a minimal `config.json` (no `project_id`).

Run `supa init` again later to connect to the platform when you're ready to deploy.

### `--project`

Link to an existing project using its reference ID. Find your project ref in the dashboard URL: `https://supabase.com/dashboard/project/<ref>`

## Examples

### Interactive setup

```bash
supa init
```

### Local development (no account needed)

Get started immediately without a Supabase account. Creates the project structure so you can start writing SQL schema files right away.

```bash
supa init --local
```

### Create new project non-interactively

For CI/CD pipelines or scripts where you can't use interactive prompts.

```bash
supa init --new --org my-org --name my-project --region us-east-1
```

### Link to existing project

```bash
supa init --link abc123xyz
```
