...
Each example below shows a complete scenario — the claim, the rule, and
the client code. Start here to see how claims and rules work together.

## User owns their data

The simplest pattern is when users can only see their own records.
Use `auth_rules.user_id_marker()` to match the authenticated user:
...
| What you want | Rule |
|---------------|------|
| User's own data | `auth_rules.eq('user_id', auth_rules.user_id_marker())` |
