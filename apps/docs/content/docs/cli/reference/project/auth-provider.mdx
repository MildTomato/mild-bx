...
- [`remove`](#remove) - Remove an OAuth provider and clear its credentials
...
```bash
supa project auth-provider remove <provider> [options]
```
...
| Argument | Required | Description |
|----------|----------|-------------|
| `provider` | Yes | Provider to remove (e.g., google, github) |
...
```bash
supa project auth-provider remove google
```
